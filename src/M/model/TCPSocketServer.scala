...
class TCPSocketServer(gameActor: ActorRef) extends Actor {
  ...
  def handleMessageFromWebServer(messageString:String):Unit = {
    ...
    messageType match {
      case "connected" => gameActor ! AddRunner(username)
      case "disconnected" => gameActor ! RemoveRunner(username)
      case "move" =>
        val x = (message \ "x").as[Double]
        val y = (message \ "y").as[Double]
        gameActor ! MoveRunner(username, x, y)
      case "stop" => gameActor ! StopRunner(username)
    }
  }
